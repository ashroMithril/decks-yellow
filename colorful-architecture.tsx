...
export default function ColorfulArchitecture() {
  ...
  useEffect(() => {
    ...
    const timer8 = setTimeout(() => setAnimationStage(8), 4800) // Multi-LLM Architecture
    const timer9 = setTimeout(() => setAnimationStage(9), 5400) // Security & Guardrails
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-white to-gray-100 text-[#281C46] p-4">
      <div className="max-w-6xl mx-auto">
          ...
          >
            <div className="bg-white border border-[#5A3BFE]/20 rounded-lg p-2 h-full shadow-sm">
              ...
              <div className="flex justify-center mb-2">
                <div className="bg-[#F8D57E] backdrop-blur-sm rounded-lg px-2 py-1 text-center w-full border border-[#F8D57E]/50 shadow-sm">
                ...
                {[
                  ...
                  >
                    <div className="bg-white backdrop-blur-sm border border-[#5A3BFE]/20 rounded-lg p-1 text-center shadow-sm">
                      <p className="text-[11px] font-medium text-[#281C46]">{integration}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-3">
                <div className="bg-[#5A3BFE] backdrop-blur-sm rounded-lg p-2 text-center border border-[#5A3BFE]/50 shadow-md mb-2">
                  <p className="text-sm font-bold text-white">MCP</p>
                  <p className="text-[10px] text-white/80">Model Context Protocol</p>
                </div>
                <div className="grid grid-cols-2 gap-1.5">
                  {["Postgres", "HubSpot", "Salesforce", "Shopify"].map((mcp, index) => (
                    <div
                      key={mcp}
                      style={{
                        transitionDelay: `${600 + 50 * index}ms`,
                        opacity: animationStage >= 5 ? 1 : 0,
                        transform: animationStage >= 5 ? "scale(1)" : "scale(0.8)",
                      }}
                      className="transition-all duration-300"
                    >
                      <div className="bg-[#5A3BFE]/10 backdrop-blur-sm border border-[#5A3BFE]/30 rounded-lg p-1 text-center shadow-sm">
                        <p className="text-[10px] font-medium text-[#281C46]">{mcp}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {[
                  ...
                  >
                    <div className="bg-[#5A3BFE] backdrop-blur-sm border border-white/20 rounded-lg p-1 text-center shadow-sm">
                      <feature.icon className="w-3 h-3 mx-auto mb-1 text-white" />
                      <p className="text-[11px] font-medium text-white">{feature.title}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-3">
                  {[
                    ...
                    { icon: Zap, title: "Automated Testing", description: "quality assurance" },
                    ...
                    >
                      <div className="bg-gray-50 backdrop-blur-sm border border-[#5A3BFE]/20 rounded-lg p-2 shadow-sm hover:shadow-md transition-all duration-300">
                        <div className="flex items-center mb-1">
                          <feature.icon className="w-4 h-4 mr-1.5 text-[#5A3BFE]" />
                          <h4 className="font-semibold text-sm text-black">{feature.title}</h4>
                        </div>
                        <p className="text-[11px] font-medium text-[#281C46] pl-5.5">{feature.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white border border-[#5A3BFE]/20 rounded-lg p-2 h-full shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  >
                    <div className="bg-white backdrop-blur-sm border border-[#5A3BFE]/20 rounded-lg p-1 text-center shadow-sm">
                      <channel.icon className="w-3 h-3 mx-auto mb-1 text-[#5A3BFE]" />
                      <p className="text-[11px] font-medium text-[#281C46]">{channel.title}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        >
          ...
          <div className="grid grid-cols-6 gap-2 mt-2">
            {["GPT-5.1", "GPT-4.5", "Claude 3.7", "Gemini 3", "Llama 4", "Custom Models"].map((model, index) => (
              <div
                key={model}
                style={{
                  transitionDelay: `${50 * index}ms`,
                ...
                className="transition-all duration-300 bg-[#F8D57E] backdrop-blur-sm border border-[#F8D57E]/50 rounded-lg p-1 text-center shadow-sm"
              >
        ...
        <div
          className={`bg-[#281C46] border border-[#281C46]/20 rounded-lg p-2 mb-3 shadow-sm transition-all duration-700 ${animationStage >= 9 ? "opacity-100 transform translate-y-0" : "opacity-0 transform translate-y-8"}`}
        >
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              ...
              <div
                ...
                className="transition-all duration-500 bg-white backdrop-blur-sm border border-white/20 rounded-lg p-2 text-center shadow-sm"
              >
